                            ...
                            if (!inputStacks.isEmpty()) {
                                ...
                                if (inputStacks.size() > 1) {
                                    ...
                                    if (optionalTag.isPresent()) {
                                        stack = new ItemStack(ClassicPipes.TAG_LABEL, inputStacks.get(0).getCount());
                                        CompoundTag compoundTag = stack.getOrCreateTag();
                                        compoundTag.putString("classic_pipes_label", optionalTag.get().location().toString());
                ...
                if (doStart) {
                    ...
                    if (filterScreen instanceof FluidFilterScreen<?>) {
                        ...
                        if (ingredient.getIngredient() instanceof IJeiFluidIngredient fluidStack) {
                            fluid = new FluidWithData(fluidStack.getFluid(), fluidStack.getTag().orElse(new CompoundTag()));
                        } else {
